...
Class Rs_model extends CI_Model
{
	...
	function connected()
	{
		var_dump($this->db->conn_id->connect_error);
	...
	*/
	...
	function select_element_by($table, $where = false, $where2 = false, $select = '*')
	{
		$result = $this->db->select($select)->where($where, $where2)->get($table)->row();
		return $result;
	}
	...
	function get_all_elements_by($table, $where = false, $where2 = false, $order_by = false, $ord_by = false, $group_by = false)
	{
		if($order_by)
			$this->db->order_by($order_by, $ord_by);
	...
	{
		$element = $this->select_element_by($table, $where, $data[$where], 'id, ' . $where);
		if($element)
		{
